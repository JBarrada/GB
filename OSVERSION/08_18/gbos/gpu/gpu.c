...
extern void write_pixel_row(unsigned int y, unsigned char *d);
extern void write_160_144(unsigned int y, unsigned int x, unsigned char *d);
extern void que_visual();
		...
		case 0: // in hblank
			if(gpu_.modeclocks >= 51) {
				...
				if(gpu_.curline == 143) { // end of hblank for last scanline
					gpu_.linemode = 1;
					// render screen
					write_160_144(62, 0, (unsigned char*)screen);
			...
			if(gpu_.modeclocks >= 43) {
				...
				if(gpu_.lcdon) { // render scanline
					if (gpu_.bgon) {
						...
						int w;
						for (w=0; w<160; w++) {
							screen[w + (160*gpu_.curline)] = gpu_.tilemap[tile][y][x]+8;
						...
						if (gpu_.bgtilebase) {
							...
							do {
								...
								screen[(160-w) + (160*gpu_.curline)] = gpu_.tilemap[tile][y][x]+8;
								x++;
								if (x==8) {
									t = (t+1)&31;
									x = 0;
							...
							do {
								...
								screen[(160-w) + (160*gpu_.curline)] = gpu_.tilemap[gpu_.vram[mapbase+t]][y][x]+8;
								x++;
								if (x==8) {
									t = (t+1)&31;
									x = 0;
										...
										for (x=0; x<8; x++) {
											if (obj_.x+x >= 0 && obj_.x+x < 160) {
												if (tilerow[7-x] && (obj_.prio || !gpu_.scanrow[x])) {
													screen[screen_x + (160*gpu_.curline)] = tilerow[7-x]+8;
												}
											}
											screen_x++;
										}
										...
										for (x=0; x<8; x++) {
											if (obj_.x+x >= 0 && obj_.x+x < 160) {
												if (tilerow[x] && (obj_.prio || !gpu_.scanrow[x])) {
													screen[screen_x + (160*gpu_.curline)] = tilerow[x]+8;
												}
											}
											screen_x++;
										}
...
void gpu_update_oam(unsigned int addr, unsigned char val) {
	...
	for (i=0; i<40; i++) {
		...
		//printf("S: %d | D: %d\n", gpu_.objdatasorted[i].x, gpu_.objdata[i].x);
	}
